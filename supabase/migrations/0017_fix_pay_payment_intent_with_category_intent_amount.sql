-- Fix: pay_payment_intent_with_category was failing with
--   record "intent" has no field "amount"
-- This happens when the PL/pgSQL variable `intent` is declared as `record` and
-- the selected column set does not include `amount` in the deployed version.
--
-- We make it robust by typing `intent` as `public.payment_intents%ROWTYPE`.
...
exception
  when others then
    -- Keep a safe message but include useful diagnostics for debugging.
    return json_build_object(
      'success', false,
