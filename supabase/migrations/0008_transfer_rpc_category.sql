-- Add a structured transfer category to transaction metadata.
-- The client sends `transfer_category` from the Send Money flow.
...
create or replace function public.transfer_user_to_user(
  ...
  transfer_category text default null,
...
begin
  ...
  if transfer_category is not null and btrim(transfer_category) <> '' then
    txn_metadata := txn_metadata || jsonb_build_object('category', btrim(transfer_category));
  end if;
...
grant execute on function public.transfer_user_to_user(uuid, numeric, text, text, uuid) to authenticated;
grant execute on function public.transfer_user_to_merchant(text, numeric, text, text, uuid) to authenticated;
